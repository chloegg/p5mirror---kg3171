//https://editor.p5js.org/kg3171/sketches/4sp9J0yL-f
//ims04-chlo.
...
function setup() {
  ...
  fsButton.position(390, 10); // Adjust position as needed
  
  function toggleFullScreen() {
    let fs = fullscreen();
    fullscreen(!fs);
    fsButton.style('display', fs ? 'none' : 'block'); // Hide button in fullscreen, show otherwise
  }
  
    let snapButton = createButton('Take Snapshot');
  snapButton.mousePressed(takeSnapshot);
  snapButton.position(390, 70);  // Adjust position as needed
}

function takeSnapshot() {
  my.videoImg = my.video.get();  // Assuming 'my.video' is your video element
  save(my.videoImg, 'snapshot.jpg');  // Save the current frame as an image file
}
